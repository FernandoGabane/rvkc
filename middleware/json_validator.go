...
func ValidateJSON(ctx *gin.Context, payload any) error {
	...
	LoggerInfo("payload received", strings.TrimSpace(strings.ReplaceAll(string(body), "\n", "")))
	if err != nil {
		return err
	}
...
func ToJson(request any) ([]byte, error) {
	return json.Marshal(request)
}

func ToJsonString(request any) (string) {
	if json, err := ToJson(request); err == nil {
		return strings.TrimSpace(string(json))
	} 

	return "it was not possible to convert the request to JSON"
}